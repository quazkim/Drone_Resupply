#!/usr/bin/env bash
# CI benchmark — wraps run_benchmark_v5.sh with scope/time filtering.
# Usage: bash scripts/run_benchmark_ci.sh <scope> <time_n10> <time_n1520> <lsp> <workers>
#   scope    : n10 | n15 | n20 | all
#   time_n10 : budget in seconds for n=10 instances (default 150)
#   time_n1520: budget for n=15/20 (default 300)
#   lsp      : CP-SAT cutoff per call (default 3)
...
TMP_DIR=$(mktemp -d)
OUT_CSV="./results/comparison_ci_$(date +%Y%m%d_%H%M).csv"

mkdir -p results

echo "Customers;Depot Location;Beta;Instance;MILP Objective;MILP CPU Time (s);GA+Tabu C_max;GA+Tabu CPU Time (s);Gap (%)" > "$OUT_CSV"
echo "[BENCH] Scope   : $SCOPE"
echo "[BENCH] T n=10  : ${T10}s"
echo "[BENCH] T n>=15 : ${T1520}s"
echo "[BENCH] LSP cut : ${LSP}s"
...
export -f run_one
export SOLVER INST_DIR TMP_DIR T10 T1520 LSP
export DEPOT_NAME

# Full MILP reference table
MILP_DATA="
...
# Filter by scope
case "$SCOPE" in
  n10)  FILTERED=$(echo "$MILP_DATA" | grep -E '^\s*10 ') ;;
  n15)  FILTERED=$(echo "$MILP_DATA" | grep -E '^\s*15 ') ;;
  n20)  FILTERED=$(echo "$MILP_DATA" | grep -E '^\s*20 ') ;;
  all)  FILTERED=$(echo "$MILP_DATA" | grep -v '^\s*$') ;;
  *)    echo "[ERROR] Unknown scope: $SCOPE (use n10|n15|n20|all)"; exit 1 ;;
esac

TOTAL=$(echo "$FILTERED" | wc -l | tr -d ' ')
echo "[BENCH] Instances: $TOTAL"

# Run in parallel
echo "$FILTERED" | xargs -P "$WORKERS" -L 1 bash -c 'run_one "$@"' _
...
while IFS=' ' read -r cust depot beta num milp_obj milp_time; do
  ...
  [[ -f "$TMP_DIR/$key" ]] && cat "$TMP_DIR/$key" >> "$OUT_CSV"
done <<< "$FILTERED"
...
for r in rows:
    g = r.get('Gap (%)', 'N/A')
    try:    gaps.append(float(g.replace('%','')))
    except: pass
...
for size in [10, 15, 20]:
    ...
    if sg:
        ...
        print(f"  n={size} avg gap  : {sum(sg)/len(sg):.2f}%  (better={b}/{len(sg)})")
PYEOF
